...
Для редактирования вам потребуется любой текстовый редактор, например [VSCode](https://code.visualstudio.com/), [Sublime Text](https://www.sublimetext.com/), Emacs, Vim, или любой другой который не зависнет на файле размером `30Mb`.

Так-же вам потребуется узнать о такой штуке как git, есть несколько программ:
- [GitHub Desktop](https://desktop.github.com/);
- [Список других программ](https://git-scm.com/download/gui/windows);
- В некоторых текстовых редакторах, может быть встроен либо иметь плагины для работы с git;
- Ещё можно это всё делать через командную строку/терминал, но это для продвинутых.

Вам так-же потребуется базовое понимания языка разметки [XML](http://citforum.ru/internet/xmlxslt/xmlxslt.shtml), на самом деле вы можете просто делать всё по примеру уже готовых глав в книге, главно примерно понимать что такое теги (`<p>текст</p>`).

Полезно так-же будет, ознакомиться со списком доступных тегов для формата `fb2`(Fiction Book 2.0):
- http://gribuser.ru/xml/fictionbook/shema_comments.html
